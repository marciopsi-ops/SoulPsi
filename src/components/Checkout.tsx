...
export function Checkout({ therapistId, profileData, bookingData, onSuccess, onCancel }: any) {
  const [phase, setPhase] = useState<'review' | 'payment-choice' | 'payment-pix' | 'success'>('review');
  ...
  const handleConfirmReservation = async () => {
    ...
    try {
      ...
      if (bookingData?.appointmentDate && bookingData?.appointmentTime) {
         datetime = new Date(`${bookingData.appointmentDate}T${bookingData.appointmentTime}`).toISOString();
      }
      ...
      if (e.message?.includes('missing or insufficient permissions')) {
        handleFirestoreError(e, OperationType.CREATE, `profiles/${therapistId}/appointments`);
      } else {
  ...
  if (phase === 'success') {
    ...
    return (
      <div className="w-full max-w-lg mx-auto py-20 px-4 flex flex-col items-center">
        ...
        <h2 className="text-2xl font-bold text-slate-800 mb-2">Sessão Solicitada com Sucesso!</h2>
        <div className="bg-amber-50 border border-amber-100 p-6 rounded-xl text-sm justify-center flex flex-col items-center mb-8 text-center w-full">
          <div className="text-amber-800 font-medium mb-4 text-left w-full">
            Confirmação enviada por e-mail e SMS. Entre em contato com o profissional pelo seu WhatsApp para:
            <ul className="list-disc pl-5 mt-2 space-y-1 text-amber-700">
              <li>confirmar a sessão, data e hora</li>
              <li>obter o link da chamada de vídeo ou instruções presenciais</li>
              <li>obter orientações de pagamento ou enviar o comprovante, caso já tenha pago.</li>
            </ul>
          </div>
          <a
          ...
          <>
            ...
            <div className="bg-slate-50 p-4 rounded-xl mb-6 flex flex-col sm:flex-row items-start sm:justify-between sm:items-center text-slate-800 gap-2">
              <div>
                ...
                {bookingData?.appointmentDate && (
                  <p className="text-sm text-slate-500 mt-1">{bookingData.appointmentDate} às {bookingData.appointmentTime}</p>
                )}
              </div>
            ...
            {(!allowPayment || !isPaid) && (
              <div className="mb-6 bg-slate-50 p-4 border border-slate-100 rounded-xl">
                <p className="text-slate-700 text-center">Para este serviço, os detalhes e formas de pagamento serão alinhados diretamente com o profissional pelo <b>WhatsApp</b> após a confirmação reserva.</p>
              </div>
            )}
        ...
        {phase === 'payment-choice' && (
          <>
            ...
            <div className="mb-6 flex flex-col gap-4">
              <p className="text-slate-700 font-medium text-center mb-2">Para agilizar, como você prefere realizar o pagamento?</p>
              
              <button onClick={() => { setPatientChoice('pix'); setPhase('payment-pix'); }} className="w-full text-left p-4 border border-emerald-200 rounded-xl hover:border-emerald-400 hover:bg-emerald-50 transition flex items-center gap-4">
              ...
              <button onClick={() => { setPatientChoice('whatsapp'); setPhase('success'); }} className="w-full text-left p-4 border border-blue-200 rounded-xl hover:border-blue-400 hover:bg-blue-50 transition flex items-center gap-4">
                 <div className="w-12 h-12 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shrink-0">
                   <span className="font-bold text-xl">Wa</span>
                 </div>
                 <div>
                   <p className="font-bold text-slate-800">Combinar pelo WhatsApp</p>
                   <p className="text-sm text-slate-500">Pagamento tratado diretamente com o profissional.</p>
                 </div>
              </button>
            </div>
          </>
        ...
        {phase === 'payment-pix' && (
          <>
            <div className="mb-8 border border-emerald-100 bg-emerald-50 rounded-xl p-6 flex flex-col items-center">
               <h3 className="text-lg font-bold text-emerald-800 mb-4 text-center">Pagamento via Pix</h3>
               
               {profileData?.pixQrCode ? (
                 <div className="bg-white p-3 rounded-xl shadow-sm mb-4">
                     <img src={profileData.pixQrCode} alt="QR Code Pix" className="w-40 h-40 object-contain" />
                 </div>
               ) : profileData?.pixKey ? (
                 <div className="bg-white p-3 rounded-xl shadow-sm mb-4">
                     <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(profileData.pixKey)}`} alt="QR Code Pix" className="w-40 h-40" />
                 </div>
               ) : null}
               ...
               {profileData?.pixKey && (
                 <div className="w-full mb-4">
                   <label className="block text-xs font-medium text-emerald-700 mb-1">Chave Pix</label>
                   <div className="flex">
                     <input readOnly value={profileData.pixKey} className="w-full bg-white border border-emerald-200 rounded-l-lg p-3 text-emerald-900 font-mono text-sm outline-none" />
                     <button type="button" onClick={handleCopyPix} className="bg-emerald-600 hover:bg-emerald-700 transition text-white px-4 rounded-r-lg flex items-center justify-center shrink-0">
                       {copied ? <CheckCircle2 className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                     </button>
                   </div>
                   {copied && <p className="text-xs text-emerald-600 mt-2">Chave Pix copiada para a área de transferência!</p>}
                 </div>
               )}
               
               <button onClick={() => setPhase('success')} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl transition mt-4">
                 Já efetuei o pagamento
               </button>
            </div>
            <button onClick={() => { setPatientChoice('whatsapp'); setPhase('success'); }} className="w-full text-slate-500 hover:text-slate-700 font-medium py-2 rounded-xl transition">
