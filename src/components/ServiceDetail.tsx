...
export function ServiceDetail({ therapistId, service, profileData, onBack }: { therapistId?: string, service: any, profileData: any, onBack: () => void }) {
  ...
  const getIcon = () => {
    if (service.category === 'empresa') return <Building className="w-8 h-8 text-emerald-500" />;
    if (service.category === 'psicologos' || service.category === 'psicologo') return <GraduationCap className="w-8 h-8 text-purple-500" />;
    if (service.category === 'igrejas') return <Church className="w-8 h-8 text-blue-500" />;
  ...
  if (service.category === 'psicologos' || service.category === 'psicologo') colorTheme = 'purple';
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className={`bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100`}>
          <div className={`${theme.bg} border-b ${theme.border} p-8 sm:p-12 text-center`}>
            ...
            <p className={`text-base sm:text-lg ${theme.text} mb-0 max-w-lg mx-auto`}>
              {service.description}
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
              {(service.duration || '') && (
                <div className="bg-slate-50 rounded-2xl p-4 flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Duração</h4>
              ...
              {service.price > 0 && (
                <div className="bg-slate-50 rounded-2xl p-4 flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Valor</h4>
