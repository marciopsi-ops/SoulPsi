...
import { Mail, Lock, Chrome, X, Loader2, ArrowRight } from 'lucide-react';
...
export function AuthModal({ onClose }: AuthModalProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/unauthorized-domain') {
        setError("O domínio atual não está autorizado. Adicione-o no painel do Firebase.");
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/email-already-in-use') errorMsg = "Este email já está em uso.";
      if (err.code === 'auth/invalid-credential') errorMsg = "Email ou senha incorretos.";
      ...
      if (err.code === 'auth/configuration-not-found') {
        errorMsg = "A autenticação por Email/Senha não está ativada. Ative-a no Console do Firebase (Authentication > Sign-in method > Email/Password) para liberar o cadastro com outros emails.";
      }
      if (err.code === 'auth/operation-not-allowed') {
        errorMsg = "O cadastro com email e senha está desabilitado no Firebase. É necessário ativar a opção 'Email/Password' no menu Authentication do Firebase Console.";
      }
      ...
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6">
          ...
          {(mode === 'login' || mode === 'register') && (
             <div className="space-y-4 mb-6">
               ...
               <div className="relative flex items-center py-2">
                 ...
                 <span className="flex-shrink-0 mx-4 text-slate-400 text-sm">ou com email</span>
                 <div className="flex-grow border-t border-slate-200"></div>
               </div>
             </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 relative">
             {loading && (
               <div className="absolute inset-0 bg-white/60 backdrop-blur-sm z-10 flex flex-col items-center justify-center rounded-xl">
                 ...
                 <span className="text-amber-600 font-medium text-sm">Aguarde...</span>
               </div>
             )}
          ...
          <div className="mt-6 text-center text-sm text-slate-500 space-y-2">
            {mode === 'login' && (
              <>
                <button onClick={() => { setMode('forgot_password'); setError(''); setMessage(''); }} className="text-amber-600 hover:text-amber-700 font-medium block w-full">Esqueci minha senha</button>
                <div>
                  Não tem uma conta? <button onClick={() => { setMode('register'); setError(''); setMessage(''); }} className="text-amber-600 hover:text-amber-700 font-medium">Cadastre-se</button>
                </div>
              </>
            )}
            {mode === 'register' && (
              <div>
                Já tem uma conta? <button onClick={() => { setMode('login'); setError(''); setMessage(''); }} className="text-amber-600 hover:text-amber-700 font-medium">Fazer login</button>
              </div>
            )}
            {mode === 'forgot_password' && (
              <button onClick={() => { setMode('login'); setError(''); setMessage(''); }} className="text-amber-600 hover:text-amber-700 font-medium block w-full">Voltar para o Login</button>
            )}
          </div>
        </div>
      </div>
