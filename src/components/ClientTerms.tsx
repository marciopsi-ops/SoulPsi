...
export function ClientTerms({
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-slate-600 mb-8 leading-relaxed">
            Sua assinatura e aceitação dos termos de contrato e LGPD foram
            registrados com sucesso.
          </p>
          <button
            onClick={onSuccess}
            className="bg-amber-500 text-white font-medium px-6 py-3 rounded-xl hover:bg-amber-600 transition w-full"
          >
  ...
  const defaultTerms =
    "Declaro para os devidos fins que li e estou ciente das regras de prestação de serviço, remuneração e termos da LGPD (Lei Geral de Proteção de Dados).";
          ...
          <div className="relative z-10 flex flex-col items-center">
            {profileData?.profilePhoto ? (
              <img
                ...
                className="w-20 h-20 rounded-full object-cover border-4 border-white/20 mb-4 shadow-lg"
              />
            ) : (
            ...
            <p className="text-amber-100 font-medium mt-1">
              Assinatura de Termos e Contrato
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8">
          <div className="bg-blue-50 text-blue-800 p-4 rounded-xl mb-8 flex items-start gap-3 border border-blue-100">
            ...
            <div className="text-sm">
              ...
              <p>
                Por favor, leia atentamente o documento abaixo e confirme seus
                dados para registrar sua concordância e assinatura digital.
              </p>
            </div>
          </div>
          ...
          <div className="mb-8 border border-slate-200 rounded-xl overflow-hidden flex flex-col max-h-[400px]">
            ...
            <div
              className="p-5 overflow-y-auto bg-slate-50 text-slate-700 text-sm whitespace-pre-wrap leading-relaxed doc-content"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Nome Completo {isCompany && "(Empresa)"}
              </label>
              <input
                type="text"
                required
                className="w-full p-3 border border-slate-300 rounded-xl focus:ring-amber-400 focus:outline-none"
                value={form.name}
                onChange={(e) => setForm({ ...form, name: e.target.value })}
                placeholder={
                  isCompany ? "Razão Social da Empresa" : "Seu nome completo"
                }
              />
            </div>
            <div>
          ...
          <label className="flex items-start gap-3 cursor-pointer group bg-slate-50 p-4 rounded-xl border border-slate-200 hover:bg-slate-100 transition mb-6">
            ...
            <div className="text-sm text-slate-700 leading-relaxed">
              Declaro que li e estou de acordo com os termos estabelecidos
              acima, prestando minha assinatura digital e concordância integral
              com as condições.
            </div>
          </label>
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
            <button
              type="submit"
              disabled={submitting}
              className="bg-amber-500 text-white font-semibold px-8 py-3 rounded-xl hover:bg-amber-600 transition flex items-center gap-2 disabled:opacity-50"
            >
              {submitting ? (
