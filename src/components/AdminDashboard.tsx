...
import { collection, query, getDocs, updateDoc, doc, serverTimestamp, setDoc, deleteDoc, getDoc, addDoc } from 'firebase/firestore';
import { Shield, Search, AlertCircle, CheckCircle2, UserX, MailPlus, Trash2, Link as LinkIcon, Copy, Settings, ExternalLink, Bell, Sparkles } from 'lucide-react';
...
const getReadjustmentAlert = (createdAtDate: Date | null) => {
  ...
  if (diffDays < -10) { 
    const nextAnniversary = new Date(today.getFullYear() + 1, birthMonth, birthDay);
...
export function AdminDashboard() {
  ...
  const [supportSettings, setSupportSettings] = useState({ phone: '', email: '', message: '' });
  ...
  const handleSendNotification = async () => {
    ...
    try {
      await addDoc(collection(db, `profiles/${notifyingProfile.id}/system_notifications`), {
        title: notifTitle,
  ...
  const handleUpdateAutoNotifications = async (profileId: string, enabled: boolean, period?: string, frequency?: string) => {
    try {
      ...
      const updateData: any = {
        ...
        autoNotifPeriod: period !== undefined ? period : (current.autoNotifPeriod || 'monthly'),
        autoNotifFrequency: frequency !== undefined ? frequency : (current.autoNotifFrequency || 'due_day'),
  ...
  useEffect(() => {
    if (selectedProfile) {
      ...
      if (selectedProfile.createdAt) {
        try {
          const d = selectedProfile.createdAt.toDate ? selectedProfile.createdAt.toDate() : new Date(selectedProfile.createdAt);
          if (!isNaN(d.getTime())) {
  ...
  const handleSaveAdminData = async () => {
    ...
    try {
      ...
      handleFirestoreError(e, OperationType.UPDATE, `profiles/${selectedProfile.id}`);
  ...
  const fetchSupportSettings = async () => {
    try {
      ...
      if (docSnap.exists()) {
        setSupportSettings({ phone: '', email: '', message: '', ...docSnap.data() });
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const executeDeleteAction = async () => {
    ...
    try {
      ...
      if (confirmDelete.type === 'profile') {
        handleFirestoreError(e, OperationType.DELETE, `profiles/${confirmDelete.id}`);
      } else {
        handleFirestoreError(e, OperationType.DELETE, `allowed_users/${confirmDelete.id}`);
      }
    } finally {
      setDeleting(false);
      ...
      <div className="mb-8 p-6 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl flex flex-col gap-4">
        <div>
           <h3 className="text-lg font-bold text-amber-900 flex items-center gap-2">
             <LinkIcon className="w-5 h-5 text-amber-600" /> Link de Vendas da Plataforma (Landing Page SaaS)
           </h3>
           <p className="text-sm text-amber-800 mt-1">
             Compartilhe este link com profissionais para que conheçam a plataforma e iniciem o teste de 7 dias gratuito.
           </p>
        </div>
        <div className="flex flex-col gap-3 w-full max-w-2xl">
          <div className="flex items-center bg-white px-4 py-3 rounded-xl border border-amber-200 shadow-sm w-full">
            <span className="text-sm text-slate-600 font-mono select-all truncate">{window.location.origin}/?saas=true</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
            <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center gap-2">
               ...
               <h2 className="font-semibold text-slate-800">Dados de Suporte (Plataforma)</h2>
            </div>
            <div className="p-4">
               <form onSubmit={handleSaveSupportSettings} className="flex flex-col gap-4">
                 <div>
                 ...
                 <div>
                   ...
                   <textarea 
                     ...
                     onChange={e => setSupportSettings({...supportSettings, message: e.target.value})}
             ...
             <div className="flex flex-col p-4 gap-4 bg-slate-50">
                {filteredProfiles.length === 0 ? (
                  ...
                  filteredProfiles.map(p => {
                     const isDefaulter = p.subscriptionStatus !== 'active' && p.subscriptionStatus !== 'trial';
                     const isTrial = p.subscriptionStatus === 'trial';
                     const isExpiredTrial = isTrial && p.trialEndsAt && new Date(p.trialEndsAt) < new Date();
                     ...
                     return (
                       <div key={p.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 hover:border-amber-200 hover:shadow transition-all flex items-start gap-4">
                         {/* Photo Column */}
                         ...
                         <div className="flex-1 min-w-0 flex flex-col gap-2.5">
                           ...
                           <div className="flex flex-wrap items-center gap-2">
                             <p className="text-base font-bold text-slate-800">{p.name || p.email || 'Profissional Sem Nome'}</p>
                             ...
                             <span className={cn(
                               ...
                               isTrial && !isExpiredTrial ? "bg-blue-50 text-blue-700 border-blue-200" :
                               "bg-rose-50 text-rose-700 border-rose-200"
                             )}>
                               {p.subscriptionStatus === 'active' ? <CheckCircle2 className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
                               {p.subscriptionStatus === 'active' ? 'Assinatura Ativa' : 
                                isTrial ? (isExpiredTrial ? 'Teste Expirado' : 'Em Teste (7 dias)') : 
                                'Acesso Bloqueado'}
                             </span>

                             {p.trialEndsAt && isTrial && (
                               <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">
                                 (Vence: {format(new Date(p.trialEndsAt), 'dd/MM/yyyy')})
                               </span>
                             )}
                           </div>
                           ...
                           <div className="flex flex-wrap items-center gap-x-3 text-xs md:text-sm font-semibold text-slate-500">
                             <span className="text-amber-600">{p.title || 'Psicólogo(a) Clínico(a)'}</span>
                             {p.specialties && (Array.isArray(p.specialties) ? p.specialties.length > 0 : typeof p.specialties === 'string') && (
                               <>
                             ...
                             <span className="text-slate-300">•</span>
                             <span className="text-slate-400 font-normal truncate select-all">{p.email || 'E-mail não informado'}</span>
                           </div>
                           ...
                           <div className="flex flex-col gap-1.5 pt-2 border-t border-slate-100">
                             <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs font-semibold text-slate-550">
                               <span className="flex items-center gap-1">
                                 <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">CPF:</span>{' '}
                                 <span className="text-slate-600">{p.cpf || 'Não informado'}</span>
                               </span>
                               <span className="flex items-center gap-1">
                                 <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">CRP:</span>{' '}
                                 <span className="text-slate-600">{p.crp || 'Não informado'}</span>
                               </span>
                               <span className="flex items-center gap-1">
                                 <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">CNPJ:</span>{' '}
                                 <span className="text-slate-600">{p.cnpj || 'Não informado'}</span>
                               </span>
                               <span className="flex items-center gap-1">
                                 <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">WhatsApp:</span>{' '}
                                 <span className="text-slate-600">{p.whatsapp || 'Não informado'}</span>
                               </span>
                               <span className="flex items-center gap-1">
                                 <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Admissão:</span>{' '}
                                 <span className="text-slate-600">{createdAtDate ? format(createdAtDate, 'dd/MM/yyyy') : 'Não informada'}</span>
                               </span>
                             </div>
                             ...
                             {p.adminNotes && (
                               <div className="flex items-start gap-1 text-[11px] text-slate-500 bg-slate-50 border border-slate-100 p-2 rounded-lg mt-0.5 italic max-w-lg">
                                 <span className="font-bold text-slate-600 not-italic shrink-0 mr-1">📝 Notas:</span>
                             ...
                             {readjustment && (
                               <div className={cn(
                                 ...
                                 readjustment.daysRemaining >= 0 && readjustment.daysRemaining <= 7
                           ...
                           <div className="mt-2.5 pt-2.5 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50 p-2.5 rounded-xl text-sans">
                             <div className="flex items-center gap-2">
                               <input 
                                 ...
                                 onChange={(e) => handleUpdateAutoNotifications(p.id, e.target.checked)}
                                 className="w-4 h-4 text-amber-500 border-slate-300 rounded focus:ring-amber-400 focus:ring-2 pointer-events-auto"
                               />
                               <label htmlFor={`autoNotif-${p.id}`} className="text-xs font-bold text-slate-700 cursor-pointer flex items-center gap-1.5 py-0.5">
                                 <Sparkles className="w-3.5 h-3.5 text-amber-500" /> Ativar Alertas Automáticos (Vencimento/Atrasos)
                               </label>
                             </div>
                             ...
                             {p.autoNotifEnabled && (
                               <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
                                 <div className="flex items-center gap-1">
                                   <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 font-sans">Plano/Período:</span>
                                   <select 
                                     value={p.autoNotifPeriod || 'monthly'}
                                     onChange={(e) => handleUpdateAutoNotifications(p.id, true, e.target.value, p.autoNotifFrequency)}
                                 ...
                                 <div className="flex items-center gap-1">
                                   <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 font-sans">Freq/Faturamento:</span>
                                   <select 
                                     value={p.autoNotifFrequency || 'due_day'}
                                     onChange={(e) => handleUpdateAutoNotifications(p.id, true, p.autoNotifPeriod, e.target.value)}
                                     className="text-[11px] border border-slate-200 bg-white rounded-lg py-1 px-1.5 font-semibold text-slate-600 focus:outline-none focus:ring-1 focus:ring-amber-400 font-sans"
                                   >
                                     <option value="3_days_before">3 dias antes</option>
                                     <option value="1_day_before">1 dia antes</option>
                                     <option value="due_day">No vencimento</option>
                                     <option value="3_days_after">3 dias de atraso</option>
                                     <option value="5_days_after">5 dias de atraso</option>
                                   </select>
                                 </div>
                               </div>
                             )}
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-xl animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h2 className="text-xl font-bold text-slate-800 font-sans">Detalhes do Profissional</h2>
              <button 
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto font-sans">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <p className="font-bold text-xl text-slate-800">{selectedProfile.name || 'Sem nome'}</p>
                  <p className="text-sm text-amber-600 font-semibold">{selectedProfile.title || 'Psicólogo(a) Clínico(a)'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 sm:col-span-1">
                  ...
                  <p className="text-sm text-slate-800 font-semibold break-all">{selectedProfile.email || 'Não informado'}</p>
                </div>
                
                <div className="col-span-2 sm:col-span-1">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Telefone / WhatsApp</p>
                  <p className="text-sm text-slate-800 font-semibold">{selectedProfile.whatsapp || selectedProfile.phone || 'Não informado'}</p>
                </div>

                <div className="col-span-2 sm:col-span-1">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">CPF</p>
                  <p className="text-sm text-slate-800 font-semibold">{selectedProfile.cpf || 'Não informado'}</p>
                </div>

                <div className="col-span-2 sm:col-span-1">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">CRP</p>
                  <p className="text-sm text-slate-800 font-semibold">{selectedProfile.crp || 'Não informado'}</p>
                </div>

                <div className="col-span-2 sm:col-span-1">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">CNPJ</p>
                  <p className="text-sm text-slate-800 font-semibold">{selectedProfile.cnpj || 'Não informado'}</p>
                </div>

                <div className="col-span-2">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Endereço</p>
                  <p className="text-sm text-slate-800 font-semibold">
                    {[selectedProfile.address, selectedProfile.neighborhood, selectedProfile.city, selectedProfile.state].filter(Boolean).join(', ') || 'Nenhum endereço informado'}
                  </p>
                </div>

                <div className="col-span-2 sm:col-span-1 pt-2 border-t border-slate-100">
                  <label className="block text-xs font-bold text-amber-600 uppercase tracking-wider mb-1">📅 Data de Admissão</label>
                  <input 
                    type="date"
                    value={admissionDate}
                  ...
                  />
                  <p className="text-[10px] text-slate-400 mt-1 italic">Vence anualmente para reajuste</p>
                </div>

                <div className="col-span-2 pt-2 border-t border-slate-100">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">📝 Anotações do Cadastro</label>
                  <textarea
                    rows={3}
                    value={adminNotes}
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-xl animate-in fade-in zoom-in-95 duration-200 font-sans">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xl font-bold text-slate-800">Nova Notificação ao Profissional</h2>
              </div>
              <button 
            ...
            <div className="p-6 space-y-4">
              <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl">
                <p className="text-xs text-slate-450 font-bold uppercase tracking-wider mb-0.5">Destinatário</p>
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center text-sm font-bold text-amber-700">
                    {notifyingProfile.name ? notifyingProfile.name.charAt(0) : 'P'}
                  </div>
                  <div>
                    <p className="text-sm font-bold text-slate-800">{notifyingProfile.name || 'Sem nome'}</p>
                    <p className="text-xs text-slate-500 break-all">{notifyingProfile.email || 'Sem email'}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Template Picker */}
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Selecionar Modelo de Alerta</label>
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const templates = getTemplatesForProfile(notifyingProfile);
                        const tmpl = templates[t.id as keyof typeof templates] || { title: '', message: '' };
              ...
              {/* Title input */}
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Título do Alerta</label>
                <input
                  type="text"
                  value={notifTitle}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Mensagem do Alerta (Será mostrada no painel do usuário)</label>
                <textarea
                  rows={4}
                  value={notifMessage}
        ...
        <div className="fixed inset-0 bg-slate-900/55 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl relative border border-slate-100 font-sans">
            ...
            <div className="flex justify-end gap-3">
              <button
                onClick={() => setConfirmDelete({ type: null, id: '', title: '', message: '' })}
                disabled={deleting}
