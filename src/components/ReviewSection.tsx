  ...
  return (
    <>
      <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100 mt-8 mb-8 text-left">
        {profileData?.useGoogleReviewsWidget &&
        profileData?.googleReviewsWidgetCode ? (
          <div className="w-full">
          ...
          <>
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="text-sm font-medium text-[rgb(var(--theme-primary))] bg-[rgb(var(--theme-primary)_/_0.05)] px-4 py-2 rounded-lg hover:bg-[rgb(var(--theme-primary)_/_0.1)] transition whitespace-nowrap"
              >
                Deixar Avaliação
              </button>
            </div>
            {reviews.length > 0 ? (
              <>
                <div className="flex items-center justify-end gap-1.5 text-[10px] uppercase font-bold tracking-wider text-slate-400 mb-2 mt-2 px-2">
                  <span>Deslize</span>
                  <svg
                    className="w-3 h-3 animate-[pulse_2s_ease-in-out_infinite]"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                <div className="flex overflow-x-auto gap-4 pb-6 snap-x snap-mandatory hide-scrollbar -mx-6 px-6 sm:mx-0 sm:px-0">
                  {reviews.map((r: any) => (
                    <div
                      key={r.id}
                      className="min-w-[280px] sm:min-w-[300px] max-w-[320px] bg-slate-50 p-5 rounded-2xl border border-slate-100 snap-center shrink-0"
                    >
            ...
            >
              <div>
                ...
                <input
                  required
                  type="text"
                  className="w-full p-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[rgb(var(--theme-primary))] focus:outline-none"
              ...
              <div>
                ...
                <textarea
                  required
                  rows={4}
                  className="w-full p-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[rgb(var(--theme-primary))] focus:outline-none resize-none"
                ...
                <p className="text-xs text-slate-500 mt-2">
                  Sua avaliação passará por moderação antes de ser exibida no
                  perfil.
                </p>
              </div>
              <button
                type="submit"
                className="w-full bg-[rgb(var(--theme-primary))] text-white font-bold py-3 px-4 rounded-xl shadow mt-2 hover:opacity-90 transition-opacity"
              >
                Enviar Avaliação
              </button>
            </form>
